...
class MedicalInsurance:
    ...
    def load_model(self):
        ...
        try:
            with open(Config.MODEL_FILE_PATH, 'rb') as f:
                self.linear_regression = pickle.load(f)
                
            with open(Config.JSON_FILE_PATH, 'r') as f:
                self.label_enc_data = json.load(f)
    ...
    def get_predict_charges(self):
        ...
        try:
            ...
            # Prepare input array
            column_names = self.linear_regression.feature_names_in_
            region_index = np.where(column_names == self.region)[0][0]
            test_array = np.zeros(self.linear_regression.n_features_in_)
            test_array[0] = self.age
            test_array[1] = self.label_enc_data['gender'][self.gender]
            test_array[2] = self.bmi
            test_array[3] = self.children
            test_array[4] = self.label_enc_data['smoker'][self.smoker]
            test_array[region_index] = 1
            
            # Make prediction
            predicted_charges = self.linear_regression.predict([test_array])
